...
```java
// Autor.java (Módulo 3, reutilizada; se agregan getId() y setNombre(),
...
1. Creá `ServicioAutores` (`@Service`), inyectando `RepositorioAutores`
   por constructor.
...
- `ServicioAutores` está anotada con `@Service` e inyecta
  `RepositorioAutores` por constructor.
